...
RUN echo "deb http://ppa.launchpad.net/ondrej/php-7.0/ubuntu trusty main" | tee -a /etc/apt/sources.list && \
    echo "deb-src http://ppa.launchpad.net/ondrej/php-7.0/ubuntu trusty main" | tee -a /etc/apt/sources.list && \
...
RUN apt-get update && \
    apt-get upgrade -y && \
    apt-get install -y --no-install-recommends php7.0-fpm php7.0-cli php7.0-json
